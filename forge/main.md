...
8) Validate the analyst output before using it. Require valid JSON with `plan_version`, `pattern`, `roles`, `artifacts`, `handoffs`, `state`, `rationale`, `risks`, `assumptions`, and `confidence`. Validate semantics as well as presence: `plan_version` must equal `forge.v2`, role names must be unique lowercase kebab-case, `pattern.type` must be one of `single-agent`, `sequential`, `parallel`, `loop`, `review-and-critique`, `iterative-refinement`, `coordinator`, `hierarchical-task-decomposition`, `swarm`, `human-in-the-loop`, or `custom-logic`, every role must expose a complete boolean `tools` object, `pattern.config.entry_role` and `pattern.config.final_output_role` must exist in `roles`, every required artifact must have exactly one producer, every required consumer must be reachable, any review or revision behavior must be represented explicitly in topology or checkpoints, and every bounded pattern must include explicit termination or approval controls. Also require an explicit file-backed artifact handoff contract: one shared workflow state for the run-level `folder-name`, one shared workflow state for the monotonic namespace-local `run-id`, deterministic JSON persistence rules for every required handoff artifact, and explicit downstream consumers that must validate and read those exact file paths before processing. Require the runtime contract to use the generated helper script for `start-run`, `write`, `validate`, and exact-path `read` operations rather than ad hoc file access. When any role may need to read a large approved directory or multi-file corpus, require `task: true` for that role and require the plan constraints or notes to say the role must use `rlm` when direct reads would be too large. Reject tool grants that are broader than the role purpose requires, except where a role needs `bash` specifically to invoke the generated artifact runtime helper or `task` specifically to use RLM for large approved reads. Reject plans that merge distinct approval, testing, accessibility, or delivery stages without preserving those semantics in roles, artifacts, or checkpoints. If validation fails, re-invoke `forge/agent-forge-analyst` with a targeted correction request. Allow at most 2 repair attempts, then fail closed.
...
10) Resolve name collisions before design. Use `forge/agent-forge-collision-resolver` for namespace folders and agent spec filenames if any collision or normalization issue exists. Keep the manifest filename fixed as `workflow.manifest.json` and the artifact helper filename fixed as `save_workflow_artifact.py`.
11) Derive a deterministic runtime artifact contract before drafting prompts. The contract must require every generated role to persist the actual artifact data it passes downstream as JSON, using a shared run-level `folder-name`, a monotonic namespace-local `run-id`, and the namespace-local directory shape `output/<folder-name>/<currentdate>/run-<zero-padded-run-id>/`. Unless the source requires another owner, the entry role chooses the shared kebab-case `folder-name` once, allocates the next incremental `run-id` exactly once via the runtime helper, and all downstream roles reuse both values unchanged. The contract must also use the canonical support script at `forge/save_workflow_artifact.py` as the source template for the generated `<namespace>/save_workflow_artifact.py`, preserving its path-safety, locking, atomic write, validation, exact-path read, and CLI shape.
12) Invoke `forge/agent-forge-designer` via the Task tool to draft OpenCode agent markdown specs from the approved `forge.v2` plan, namespace path, runtime artifact contract, and any resolved filename map you already established.
13) Validate the designer output before using it. Require valid JSON with `agents`, and verify every agent includes a stable `agent_id`, kebab-case markdown filename, description, full markdown body, and a complete boolean `tools` object. Reject duplicate ids or filenames. Apply a deterministic runtime validation pass rather than relying only on helper self-checks: parse frontmatter, verify the allowed frontmatter keys (`description`, `mode`, `hidden`, `temperature`, `steps`, `tools`, `permission`), verify tool keys and boolean values, reject prompts that contain unresolved placeholders or unsafe write instructions, require concrete sections for role behavior and artifact ownership, and reject unknown or over-broad permission metadata. When the runtime artifact contract is enabled, also require each prompt to mention the exact helper script path, the exact run allocation and reuse rules, the required artifact path shape `output/<folder-name>/<currentdate>/run-<zero-padded-run-id>/`, the exact upstream artifact path or filename mapping it must validate and read, the obligation to persist the actual downstream artifact data as JSON via the helper, the obligation for downstream consumers to validate and read required artifact file paths before processing, the no-delete/no-cleanup rule, and the obligation to return the saved artifact path or paths. If validation fails, re-invoke `forge/agent-forge-designer` with a targeted correction request. Allow at most 2 repair attempts, then fail closed.
14) Resolve every final output filename inside the chosen namespace path. Bind each `agent_id` to exactly one final relative output path before review.
15) Synthesize a workflow orchestration manifest at `<namespace>/workflow.manifest.json` from the validated `forge.v2` plan, resolved paths, and approved runtime constraints. The manifest must be machine-readable and must describe topology, entry role, role-to-file mapping, artifacts, handoffs, state, checkpoints, stopping rules, and the shared file-backed artifact handoff contract.
16) Validate the synthesized orchestration manifest. Require valid JSON with `manifest_version`, `namespace_path`, `reference_patterns`, `pattern`, `entry_role`, `resolved_agents`, `artifacts`, `handoffs`, `state`, `human_checkpoints`, `topology`, and `final_outputs`. Copy `pattern.reference_patterns` from the plan into top-level `reference_patterns` so the manifest records the same source taxonomy. Validate exact one-to-one correspondence between `agents`, `resolved_agents`, and manifest role bindings. Reject unsupported topology shapes, unbounded loops, missing convergence rules, missing approvals only for patterns that require them, lossy omission of required revision paths, duplicated checkpoint structures that diverge in meaning, unsafe graph paths to write-capable roles, or any manifest that fails to encode the shared artifact `folder-name` state, the shared monotonic `run-id` state, deterministic JSON artifact persistence, exact artifact path contracts, runtime validation requirements, and the rule that downstream consumers must validate and read required artifact files.
17) Synthesize the support script at `<namespace>/save_workflow_artifact.py` from the runtime artifact contract by adapting the canonical `forge/save_workflow_artifact.py` template. Preserve the template's safety behavior unless the user explicitly requires a narrower variant. The script must be deterministic, allocate monotonic incremental `run-id` values under a single-machine lock, derive the current date at runtime, write only inside `<namespace>/output/<folder-name>/<currentdate>/run-<zero-padded-run-id>/`, sanitize `folder-name`, `artifact-name`, and any optional sequence key, write actual artifact payloads as JSON envelopes using atomic temp-file-then-rename behavior, validate artifacts before downstream use, support exact-path reads, and never delete prior outputs. Generate it as a Python CLI with subcommands for at least `start-run`, `write`, `validate`, and `read`, and print structured JSON results on success.
18) Invoke `forge/agent-forge-critic` via the Task tool to review the full design against agentic design patterns and operational safety, using the exact final relative output paths, synthesized orchestration manifest, runtime artifact contract, and support script that would be written.
19) If the critic does not approve the design, revise the design by applying the critic feedback and re-run the critic once before writing files. Do not write unapproved specs, support files, or manifests.
20) Before writing files, perform one final deterministic validation pass on the approved specs, approved paths, support script, and orchestration manifest. If any spec, script, or manifest fails validation, stop instead of writing partial output.
...
- In the final result, report every written file path relative to the agents directory, including the support script path and the manifest path.
...
- Use the shared schema version `forge.v2` for summaries, plans, orchestration manifests, and persisted runtime artifacts.
...
- By default, generated workflows must preserve one shared run-level `folder-name`, one shared monotonic namespace-local `run-id`, and require each role to persist the actual artifact data it passes downstream as JSON into `output/<folder-name>/<currentdate>/run-<zero-padded-run-id>/` before emitting final artifacts.
...
- Generated prompts should name the upstream artifact files and validation commands concretely whenever the artifact set is known, such as `project-brief.json` or `research-insights.json`, rather than only referring to artifacts abstractly.
